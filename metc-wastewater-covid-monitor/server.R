  ...
  output$loadPlot <-
    renderPlotly({
      load_data %>%
        ...
        add_trace(
          x = ~ slice(load_data, 1:(n() - 27))$date,
          y = ~ slice(load_data, 1:(n() - 27))$covid_cases_7day,
          name = "7-day avg. cases per capita, Metro Service Area",
          fill = "tozeroy",
          fillcolor = "rgba(160, 160, 160, .3)",
          line = list(width = 0.5, color = colors$suppGray),
          hoverinfo = "text",
          text = ~ slice(load_data, 1:(n() - 27))$hover_text_case
        ) %>%
        add_trace(
          x = ~ slice(load_data, (n() - 26):n())$date,
          y = ~ slice(load_data, (n() - 26):n())$covid_cases_7day,
          name = "7-day avg. cases per capita, Metro Service Area, Incomplete",
          fill = "tozeroy",
          fillcolor = "rgba(160, 160, 160, .8)",
          line = list(width = 0.5, color = colors$suppGray),
          hoverinfo = "text",
          text = ~ slice(load_data, (n() - 26):n())$hover_text_case
        ) %>%
        layout(
