...
### copy data
```{r load}
raw_load_data <- read_excel("data/raw_load_data.csv",
  col_types = c(
    "text", "skip", "skip",
...
```{r}
case_data <- read.csv('https://static.usafacts.org/public/data/covid-19/covid_confirmed_usafacts.csv?_ga=2.86006619.233414847.1642517751-2016304881.1642174657') %>%
  janitor::clean_names() %>%
  ...
  pivot_longer(cols = contains("x"), names_to = 'date', values_to = 'covid_cases_7day') %>%
  mutate(date = str_remove(date, "x")) %>%
  mutate(date = as.Date(date, format = "%Y_%m_%d")) %>%
  group_by(date) %>%
  summarize(covid_cases_7day = sum(covid_cases_7day)) %>%
